...
import { iAutorePost } from '../modules/autore-post';
...
export class UsersService {
  ...
  users = [
    ...
    array_finale_users: iAutorePost[]= []

  constructor(private todo_s:TodosService) { }

  get_user = (): void => {
    this.users.forEach((user) => {
      let todo_return: iTodo = this.todo_s.todos.find((t) => user.id == t.userId)!;
      this.array_finale_users.push({
        id: user.id,
        ...
        image: user.image,
        title: user.title,
        completed:todo_return.completed,
        todo:todo_return.todo
      });
    });
  };
}
